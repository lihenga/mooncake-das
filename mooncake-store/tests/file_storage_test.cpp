...
class FileStorageTest : public ::testing::Test {
   protected:
    ...
    void SetPinnedRestoreArena(FileStorage& fileStorage, void* address,
        ...
        fileStorage.pinned_restore_arena_.data = static_cast<char*>(address);
        fileStorage.pinned_restore_arena_.capacity = size;
        fileStorage.pinned_restore_arena_.mapped = device_address != nullptr;
        fileStorage.pinned_restore_arena_.device_data = device_address;
...
TEST_F(FileStorageTest, AllocatePinnedStagingBufferReleasesSharedArena) {
    auto file_storage_config = FileStorageConfig::FromEnvironment();
    file_storage_config.storage_filepath = data_path;
    file_storage_config.local_buffer_size = 128 * 1024 * 1024;
    constexpr size_t kArenaSize = 2 * 1024 * 1024;
    std::vector<char> restore_arena(kArenaSize + 4096);
    std::vector<char> device_alias_arena(kArenaSize);
    FileStorage fileStorage(file_storage_config, nullptr, "localhost:9003");
    const auto arena_begin =
        (reinterpret_cast<uintptr_t>(restore_arena.data()) + 4095) & ~4095ULL;
    SetPinnedRestoreArena(fileStorage, reinterpret_cast<void*>(arena_begin),
                          kArenaSize, device_alias_arena.data());

    auto allocation = fileStorage.AllocatePinnedStagingBuffer(kArenaSize);
    ASSERT_TRUE(allocation);
    EXPECT_EQ(reinterpret_cast<uintptr_t>(allocation->ptr()), arena_begin);
    EXPECT_EQ(allocation->device_ptr(), device_alias_arena.data());
    EXPECT_EQ(allocation->size(), kArenaSize);
    EXPECT_FALSE(fileStorage.AllocatePinnedStagingBuffer(1));

    allocation.reset();
    EXPECT_TRUE(fileStorage.AllocatePinnedStagingBuffer(kArenaSize));
}
