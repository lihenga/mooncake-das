...
class TransferEngine::ScatterTransferOperation::Impl {
   public:
    ...
    Status wait() {
        ...
        // Skip recording when wait() is called from ~Impl() after the
        // operation already completed — those calls have wait_us≈0 and
        // poll_count=0, which would dilute the aggregated averages.
        if (poll_count > 0) {
